...
Run `add` only from a project containing `anhedral.json` schema v4:
...
For manual projects without a trustworthy schema-v4 `anhedral.json`, add modules with the manual reference instead. Never fabricate manifest ownership records, template provenance, or hashes. A manually authored workspace is valid as an application workspace but is not CLI-managed until it has been regenerated by a matching Anhedral version.
